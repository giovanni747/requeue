...
// Generate consistent colors for users based on their socketId
const getUserColorHex = (socketId: string) => {
  const colors = [
    '#3b82f6', // blue
    '#10b981', // green
    '#ef4444', // red
...
// Smooth cursor component for each user
function SmoothCursor({ 
  ...
  const lastMousePos = useRef<Position>({ x: cursor.x, y: cursor.y });
  ...
  const cursorX = useSpring(cursor.x, springConfig);
  const cursorY = useSpring(cursor.y, springConfig);
  ...
  }, [cursor.x, cursor.y, cursorX, cursorY, rotation, scale]);
  ...
  return (
    <>
      {Array.from(cursors.values()).map((cursor) => {
        const colorHex = getUserColorHex(cursor.socketId);
